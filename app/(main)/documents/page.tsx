  ...
  const handleFileUpload = async (files: FileList) => {
    ...
    try {
      ...
      if (!res.ok || !data?.success || !data.documents) {
        const message = data?.error || data?.failures?.[0]?.error || 'Upload failed';
  ...
  return (
    <div className="mx-auto max-w-5xl">
      <div className="mb-8 flex items-center justify-between">
        <h1 className="flex items-center gap-3 text-4xl font-bold text-gray-800 dark:text-white">
          Documents
        </h1>
        <button
          disabled={uploading}
      ...
      {documents.length === 0 ? (
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
          {documents.map(doc => (
            <div key={doc.id} className="rounded-3xl border border-gray-200 bg-white p-6 transition hover:shadow-xl dark:border-gray-700 dark:bg-gray-900">
              ...
              <div className="mt-6 flex gap-2">
                <button className="flex flex-1 items-center justify-center gap-2 rounded-2xl bg-gray-100 py-3 text-sm hover:bg-violet-100 dark:bg-gray-800">
                  <MessageSquare className="h-4 w-4" />
                  Ask about this
                </button>
                <button className="rounded-2xl px-4 text-red-500 hover:bg-red-50">
